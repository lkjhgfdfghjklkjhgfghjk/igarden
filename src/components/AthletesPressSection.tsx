...
import { PRESS_QUOTES, FEATURED_LOGOS } from '../data';
import { useI18n, getLocalizedAthletes } from '../i18n';

export const AthletesPressSection: React.FC = () => {
  const { t, language } = useI18n();
  const athletes = getLocalizedAthletes(language);
          ...
          {athletes.map((athlete, idx) => (
            <div key={idx} className="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-all border border-gray-100 flex flex-col">
              <div className="aspect-[16/10] w-full overflow-hidden bg-gray-100">
                <img referrerPolicy="no-referrer" src={athlete.image} alt={athlete.name} className="w-full h-full object-cover" />
              </div>
              <div className="p-6 sm:p-7 flex-1 flex flex-col justify-between space-y-4">
                <div className="flex items-center gap-3.5">
                  <img referrerPolicy="no-referrer" src={athlete.avatar} alt={athlete.name} className="w-12 h-12 rounded-full object-cover border-2 border-blue-100" />
                  <div>
            ...
            {[...PRESS_QUOTES, ...PRESS_QUOTES, ...PRESS_QUOTES].map((item, idx) => (
              ...
              >
                <div className="w-14 h-14 bg-white rounded-xl flex items-center justify-center p-2 shadow-xs shrink-0">
                  <img referrerPolicy="no-referrer" src={item.logo} alt={item.name} className="max-h-full max-w-full object-contain" />
                </div>
          ...
          <div className="overflow-hidden relative w-full py-2">
            <div className="flex gap-5 animate-marquee-fast hover:pause items-center">
              {[...FEATURED_LOGOS, ...FEATURED_LOGOS, ...FEATURED_LOGOS].map((logo, idx) => (
                ...
                >
                  <img referrerPolicy="no-referrer" src={logo.logo} alt={logo.name} className="max-h-full max-w-full object-contain" />
                </div>
              ))}
            </div>
          </div>
