...
export const FloatingWidgets: React.FC = () => {
  ...
  const [chatHistory, setChatHistory] = useState<{ sender: 'user' | 'bot'; text: string }[]>([
    { sender: 'bot', text: 'Bonjour ! Avez-vous des questions sur notre système de nage à contre-courant iGarden Swim Jet (1 000 W pour 209,00 €) ? Notre équipe d\'experts est à votre disposition.' }
  ]);
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setChatHistory((prev) => [
        ...
        {
          sender: 'bot',
          text: "Merci pour votre message ! Un conseiller iGarden vous répond dans quelques instants. Pour profiter directement de notre remise officielle de -50% et de la livraison express offerte, vous pouvez cliquer sur 'Commander maintenant'."
        }
      ]);
    }, 800);
  };
        ...
        {showChat && (
          <div className="absolute bottom-14 right-0 w-[300px] sm:w-[360px] bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col h-[440px] animate-in slide-in-from-bottom-5 duration-200">
            <div className="bg-[#0071E3] p-3.5 text-white flex items-center justify-between">
              <div>
                <div className="font-bold text-[15px]">Service Client iGarden France & Europe</div>
                <div className="text-[11px] opacity-80 flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-green-400 inline-block" /> Disponible 7j/7
                </div>
              </div>
              <button
                type="button"
        ...
        <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="relative w-full max-w-[650px] bg-white rounded-2xl overflow-hidden shadow-2xl flex flex-col md:flex-row border border-gray-200">
            ...
            <div className="w-full md:w-1/2 p-5 sm:p-6 flex flex-col justify-center space-y-3.5">
              <div>
                <span className="text-[11px] font-bold text-[#0071E3] uppercase tracking-wider">Offre de Lancement</span>
                <h3 className="text-[20px] font-extrabold text-gray-900 leading-tight mt-0.5">
                  Hydro-Propulseur Swim Jet 1 000 W à 209,00 €
                </h3>
                <p className="text-[12px] text-gray-500 mt-1">
                  Profitez de 50% de réduction immédiate + livraison express offerte en France et dans toute l'Europe.
                </p>
              </div>
              ...
              {!klaviyoSuccess ? (
                ...
                <div className="text-center py-3 space-y-2.5">
                  ...
                  <h4 className="font-bold text-[16px] text-gray-900">Remise activée !</h4>
                  <p className="text-[12px] text-gray-600">
                    Votre remise exceptionnelle de <strong>50%</strong> est appliquée à votre commande.
                  </p>
                  <button
                    type="button"
                    onClick={() => {
                      trackTikTokInitiateCheckout({
                        id: 'swim-jet-1000w',
                        name: 'Hydro-Propulseur de Natation iGarden Swim Jet — 1 000 W',
                        price: 209,
                        quantity: 1,
                        currency: 'EUR'
                      });
